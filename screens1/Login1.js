...
export default ({ navigation }) => {
    ...
    const handleLogin = async () => {
        try {
            ...
            const response = await axios.post('https://kgvapp.pureprakruti.com/api/v1/auth/login', requestData);

            if (response.status === 200) {
                Alert.alert('Success', 'Login successful');
                navigation.navigate('Welcome1', { user: response.data });
        ...
        <LinearGradient colors={['#545a2c', "#FFF"]} style={{ flex: 1 }}>
            <SafeAreaView style={{ flex: 1, padding: 40 }}>
                ...
                >
                    ...
                    <TouchableOpacity onPress={() => navigation.navigate('ForgotPin1')}>
