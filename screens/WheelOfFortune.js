...
class WheelOfFortune extends Component {
    ...
    _textRender = (x, y, number, i) => {
        ...
        const fontSize = number.length > 20 ? 18 : 20; // Reduce font size if the text is longer
        const lines = [];

        // Split text into multiple lines if it's too long
        if (number.length > 20) {
            const firstLine = number.slice(0, Math.floor(number.length / 2));
            const secondLine = number.slice(Math.floor(number.length / 2));
        ...
        return (
            <Text
                x={x}
                y={y - 40}
                fill={this.props.options.textColor ? this.props.options.textColor : '#fff'}
                textAnchor="middle"
                fontSize={fontSize}
            >
                {lines.map((line, index) => (
                    <TSpan
                        x={x} // Keep text centered
                        dy={index === 0 ? 0 : fontSize * 1.2} // Adjust line spacing
                        key={`arc-${i}-line-${index}`}
                    >
                        {line}
                    </TSpan>
                ))}
                    ...
                    <Image
                        source={
                            this.props.options.knobSource
                                ? this.props.options.knobSource
                                : require('../assets/images/file (2).png')
                        }
                        style={{ width: knobSize *0.9, height: (knobSize * 100) / 107, marginTop:37 }}
                    />
                </Svg>
            </Animated.View>
        );
